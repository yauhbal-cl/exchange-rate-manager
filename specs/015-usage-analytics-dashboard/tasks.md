...
- [X] T041 Confirm the feature scope boundary against `git diff --name-status $(git merge-base main HEAD)..HEAD`: implementation changes remain under `frontend/src/app/features/usage-analytics/`, except for the user-approved removal of the redundant standalone AI Insight surface (`app.routes.ts`, `app.routes.spec.ts`, `shell/shell.html`, `shell/shell.spec.ts`, and deletion of `features/ai-insight/ai-insight.ts`); feature documentation remains under `specs/015-usage-analytics-dashboard/`. Confirm no changes to `frontend/src/app/api-client/`, `contracts/openapi.yaml`, or `backend/`. Unrelated uncommitted tooling changes under `.agents/`, `.claude/`, and `.specify/` are excluded from the feature commit diff (FR-020, plan.md → Project Structure, user-approved scope amendment).
