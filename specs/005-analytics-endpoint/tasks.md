...
- [X] T023 Run `cd backend && ./mvnw verify` to confirm all new and existing tests pass (Testcontainers-backed)
- [X] T024 Execute all `quickstart.md` validation steps against a running instance (default/explicit trend windows, invalid range, unknown currency, ranked usage, recency filter, parameter rejection, usage-counter side-effect check) and confirm expected responses
